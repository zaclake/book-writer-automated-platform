  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 404) {
        ...
        response = await fetch(`${backendBaseUrl}/v2/auth/register`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name, email, password }),
        })
    ...
    <div className="min-h-screen bg-brand-sand flex items-center justify-center px-4 sm:px-6 md:px-8">
      <div className="w-full max-w-md md:max-w-lg bg-white rounded-xl shadow-lg p-6 sm:p-8">
        ...
        <p className="text-sm text-gray-600 mb-6">
          Keep your in-progress books private and organized.
        </p>
        ...
        <p className="text-sm text-gray-600 mt-6 text-center">
          ...
          <Link href="/sign-in" className="text-brand-soft-purple hover:underline">
            Sign in
          </Link>
        </p>
      </div>
