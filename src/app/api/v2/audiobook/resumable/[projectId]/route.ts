...
export async function GET(
  ...
  try {
    ...
    const targetUrl = `${backendBaseUrl}/v2/audiobook/resumable/${params.projectId}`
    ...
    return NextResponse.json(
      { error: 'Resumable check failed', details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    )
  }
}
