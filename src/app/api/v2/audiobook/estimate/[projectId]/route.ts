...
export async function GET(
  ...
  try {
    ...
    const targetUrl = `${backendBaseUrl}/v2/audiobook/estimate/${params.projectId}`
    ...
    return NextResponse.json(
      { error: 'Estimate request failed', details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    )
  }
}
