...
export async function POST(
  ...
  try {
    ...
    const targetUrl = `${backendBaseUrl}/v2/audiobook/preview/${params.projectId}`
    ...
    return NextResponse.json(
      { error: 'Preview request failed', details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    )
  }
}
