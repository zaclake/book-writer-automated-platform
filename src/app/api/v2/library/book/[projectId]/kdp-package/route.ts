...
export async function GET(request: NextRequest, { params }: { params: { projectId: string } }) {
  ...
  const targetUrl = `${backendBaseUrl}/v2/library/book/${encodeURIComponent(params.projectId)}/kdp-package`
  ...
  const respHeaders: Record<string, string> = {
    ...
    'Content-Disposition': res.headers.get('Content-Disposition') || `attachment; filename="kdp-package-${params.projectId}.zip"`,
