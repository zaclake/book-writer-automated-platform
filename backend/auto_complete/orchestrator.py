...
class AutoCompleteBookOrchestrator:
    ...
    def _build_chapter_context(self, chapter_number: int) -> Dict[str, Any]:
        ...
        if references_dir.exists():
            reference_files = {
                ...
                'entity_registry': 'entity-registry.md',
                'relationship_map': 'relationship-map.md',
                # Proposal 3 + 8 — craft references generated by the upgraded
                # ReferenceContentGenerator. Loaded into the same flat
                # `<key>_reference` slots so downstream consumers (blueprint
                # generator, narrator-sensibility injection, director-guide)
                # see them via generation_context["references"].
                'character_contradictions': 'character-contradictions.md',
                'thematic_spine': 'thematic-spine.md',
                'narrator_sensibility': 'narrator-sensibility.md',
                'subtext_bible': 'subtext-bible.md',
            }
        ...
        # Proposal 1 — surface prior chapter's plan-compliance critic findings
        # so the next chapter can acknowledge / recover from drift on-page.
        # The critique is persisted by the post-chapter critic; we render it
        # here into `recent_drift_findings` for the blueprint generator and
        # downstream prompts. Without this load, drift never propagates
        # across chapters even though the critic ran.
        if chapter_number > 1:
            try:
                ...
                prior_critique_path = (
                    ...
                    / ".project-state"
                    / "chapter-critiques"
                    / f"chapter-{chapter_number - 1:02d}.json"
                )
                if prior_critique_path.exists():
                    prior_critique = json.loads(prior_critique_path.read_text(encoding="utf-8"))
                    drift_hint = format_critique_for_next_chapter(
                        prior_critique, chapter_number=chapter_number - 1
                    )
                    if drift_hint:
                        context["recent_drift_findings"] = drift_hint
                        self.logger.info(
                            f"Loaded drift findings from Chapter {chapter_number - 1} "
                            f"critique for Chapter {chapter_number}."
                        )
            except Exception as drift_load_err:
                self.logger.debug(
                    f"Could not load prior critique for Chapter {chapter_number}: {drift_load_err}"
                )
    ...
    async def _run_post_chapter_housekeeping(
        self,
        *,
        orchestrator,
        chapter_number: int,
        ...
        """Post-chapter pipeline: facts ledger, promises ledger, plan-compliance critic.

        Centralized so both the skeleton-expand success path and the
        non-skeleton (scene-by-scene / single-pass) success path produce the
        same artifacts (Proposals 1, 4). All steps are best-effort; failures
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to update established facts for Chapter {chapter_number}: {facts_err}"
            )

        # Proposal 4 — promises ledger.
        promises_ledger = None
        try:
            from backend.auto_complete.helpers.promises_ledger import (
                PromisesLedger, apply_chapter_to_ledger,
            )
            ledger_path = self.project_path / ".project-state" / "book-promises.json"
            promises_ledger = PromisesLedger.load(ledger_path)
            blueprint_raw = generation_context.get("_blueprint_raw") or {}
            chapter_plan_for_ledger = self._get_chapter_plan(chapter_number) or {}
            ledger_chapter = {
                "promises_planted": (
                    blueprint_raw.get("promises_planted")
                    or chapter_plan_for_ledger.get("promises_planted")
                    or []
                ),
                "promises_paid": (
                    blueprint_raw.get("promises_paid")
                    or chapter_plan_for_ledger.get("promises_paid")
                    or []
                ),
            }
            paid, planted = apply_chapter_to_ledger(
                promises_ledger,
                chapter_plan=ledger_chapter,
                chapter_number=chapter_number,
            )
            if paid or planted:
                self.logger.info(
                    f"Promises ledger updated for Chapter {chapter_number}: "
                    f"paid={paid}, planted={planted}"
                )
        except Exception as promises_err:
            self.logger.warning(
                f"Failed to update promises ledger for Chapter {chapter_number}: {promises_err}"
            )

        # Proposal 1 — plan-compliance critic.
        try:
            from backend.auto_complete.helpers.plan_compliance_critic import (
                critique_chapter,
                persist_critique,
            ...
            blueprint_for_critic = generation_context.get("_blueprint_raw") or {}
            cached_bon_critique = generation_context.pop("_bon_winning_critique", None)
            if cached_bon_critique:
                critique = cached_bon_critique
            else:
                critique = await critique_chapter(
                    orchestrator=orchestrator,
                    chapter_number=chapter_number,
                    chapter_text=chapter_content,
                    blueprint=blueprint_for_critic,
                )
            persist_critique(self.project_path, chapter_number, critique)
            if promises_ledger is not None:
                reverted = reconcile_promises_with_critique(
                    promises_ledger=promises_ledger,
                    critique=critique,
                    chapter_number=chapter_number,
                )
                if reverted:
                    self.logger.warning(
                        f"Plan-compliance critic reverted promises {reverted} "
                        f"as unpaid for Chapter {chapter_number}."
                    )
            drift_hint = format_critique_for_next_chapter(
                critique, chapter_number=chapter_number
            )
            if drift_hint:
                context["recent_drift_findings"] = drift_hint
            self.logger.info(
                f"Plan-compliance critic for Chapter {chapter_number}: "
                f"score={critique.get('score', 1.0):.2f}, "
                f"findings={len(critique.get('findings') or [])}"
            )
        except Exception as critic_err:
            self.logger.debug(
                f"Plan-compliance critic skipped for Chapter {chapter_number}: {critic_err}"
            )
    ...
    async def _generate_chapter_with_references(self, chapter_number: int, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            generation_context = {
                ...
                # Proposal 1 — drift hint surfaced from prior chapter's
                # plan-compliance critic (medium/high severity findings only).
                "recent_drift_findings": context.get("recent_drift_findings", ""),
            ...
            try:
                ...
                # Proposal 4 — load the promises ledger and render the
                # outstanding-promises block for this chapter so the blueprint
                # generator (and downstream drafter) can see what payoffs are
                # owed.
                outstanding_block = ""
                try:
                    from backend.auto_complete.helpers.promises_ledger import (
                        PromisesLedger, compose_outstanding_block,
                    )
                    ledger_path = self.project_path / ".project-state" / "book-promises.json"
                    promises_ledger = PromisesLedger.load(ledger_path)
                    outstanding_block = compose_outstanding_block(
                        promises_ledger, chapter_number=chapter_number
                    )
                    generation_context["_promises_ledger_path"] = str(ledger_path)
                    generation_context["promises_outstanding_block"] = outstanding_block
                except Exception as ledger_err:
                    self.logger.debug(
                        f"Promises ledger unavailable for Chapter {chapter_number}: {ledger_err}"
                    )

                # Proposal 3 — surface the thematic-spine reference so the
                # blueprint can ground its thematic_move in the central
                # question.
                thematic_spine_ref = (
                    generation_context.get("references", {}).get("thematic-spine", "")
                    or generation_context.get("references", {}).get("thematic_spine", "")
                    or ""
                )

                # Append the prior-chapter drift hint to the anti_pattern
                # context the blueprint generator sees, so the next chapter
                # acknowledges the drift instead of repeating it.
                drift_hint = generation_context.get("recent_drift_findings", "") or ""
                anti_pattern_with_drift = anti_pattern
                if drift_hint:
                    anti_pattern_with_drift = (
                        f"{anti_pattern}\n\n{drift_hint}" if anti_pattern else drift_hint
                    )
                ...
                blueprint = await generate_chapter_blueprint(
                    ...
                    anti_pattern_context=anti_pattern_with_drift,
                    style_guide=style_guide,
                    outstanding_promises_block=outstanding_block,
                    thematic_spine=thematic_spine_ref,
                )
                ...
                self.logger.info(
                    ...
                    f"tier={blueprint.get('chapter_tier')}, move={blueprint.get('thematic_move')}"
                )
            except Exception as e:
            ...
            if use_skeleton:
                try:
                    ...
                    # Proposal 9 — Best-of-N at setpiece chapters. When the
                    # blueprint tags this chapter as a setpiece (climax,
                    # decisive confrontation, major reveal), spend extra
                    # compute: generate N drafts in parallel and let the
                    # plan-compliance critic pick the strongest.
                    blueprint_for_bon = generation_context.get("_blueprint_raw") or {}
                    use_best_of_n = (
                        os.getenv("USE_BEST_OF_N_SETPIECE", "true").lower() == "true"
                    )
                    chapter_content: str = ""
                    if use_best_of_n:
                        try:
                            ...
                            should_run_best_of_n = lambda _b: False  # type: ignore
                            run_best_of_n = None  # type: ignore
                            persist_best_of_n_artifacts = None  # type: ignore

                        if run_best_of_n is not None and should_run_best_of_n(blueprint_for_bon):
                            try:
                                from backend.auto_complete.helpers.plan_compliance_critic import (
                                    critique_chapter,
                                )
                            except Exception:
                                critique_chapter = None  # type: ignore

                            try:
                                bon_n = max(1, int(os.getenv("BEST_OF_N_SETPIECE", "2")))
                            except Exception:
                                bon_n = 2

                            async def _gen_attempt() -> str:
                                # Each attempt gets its own deep-ish copy of
                                # the generation context to avoid cross-attempt
                                # mutation of in-flight state caches.
                                ctx_copy = dict(generation_context)
                                return await generate_chapter_skeleton_expand(
                                    orchestrator=orchestrator,
                                    chapter_number=chapter_number,
                                    total_chapters=self.config.target_chapter_count,
                                    target_words=target_words,
                            ...
                            async def _crit(text: str) -> Dict[str, Any]:
                                if critique_chapter is None:
                                    return {"score": 1.0, "summary": "", "findings": []}
                                return await critique_chapter(
                                    orchestrator=orchestrator,
                                    chapter_number=chapter_number,
                                    chapter_text=text,
                                    blueprint=blueprint_for_bon,
                                )

                            self.logger.info(
                                f"Setpiece detected for Chapter {chapter_number}; "
                                f"running best-of-N with n={bon_n}."
                            )
                            try:
                                bon_result = await run_best_of_n(
                                    n=bon_n,
                                    generate=_gen_attempt,
                                    critique=_crit,
                                )
                                chapter_content = bon_result.winner.content
                                if persist_best_of_n_artifacts is not None:
                                    persist_best_of_n_artifacts(
                                        self.project_path, chapter_number, bon_result
                                    )
                                self.logger.info(
                                    f"Best-of-N selected candidate {bon_result.winner.index} "
                                    f"for Chapter {chapter_number} "
                                    f"(score={bon_result.winner.score:.2f}, "
                                    f"words={bon_result.winner.word_count})"
                                )
                                # Cache the winning critique so the post-gen
                                # critic step doesn't re-run the same call.
                                if bon_result.winner.critique:
                                    generation_context["_bon_winning_critique"] = (
                                        bon_result.winner.critique
                                    )
                            except Exception as bon_err:
                                self.logger.warning(
                                    f"Best-of-N failed for Chapter {chapter_number}: "
                                    f"{bon_err}; falling back to single attempt."
                                )
                                chapter_content = ""
                    ...
                    if chapter_content and len(chapter_content.split()) > 200:
                        await self._run_post_chapter_housekeeping(
                            orchestrator=orchestrator,
                            chapter_number=chapter_number,
                            chapter_content=chapter_content,
            ...
            if result.success:
                ...
                try:
                    ...
                    context["_last_llm_metadata"] = {}
                # Proposals 1 + 4 — also fire on the non-skeleton path so the
                # promises ledger and plan-compliance critic stay in sync no
                # matter which generator produced the chapter.
                try:
                    await self._run_post_chapter_housekeeping(
                        orchestrator=orchestrator,
                        chapter_number=chapter_number,
                ...
                except Exception as housekeeping_err:
                    self.logger.warning(
                        f"Post-chapter housekeeping failed (non-skeleton path) "
                        f"for Chapter {chapter_number}: {housekeeping_err}"
                    )
                return result.content
            else:
